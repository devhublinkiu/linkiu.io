...
function AvatarFallback({
  ...
  return (
    <AvatarPrimitive.Fallback
      ...
      className={cn(
        "flex size-full items-center justify-center rounded-full bg-slate-100 text-sm text-slate-500 group-data-[size=sm]/avatar:text-xs",
        className
      )}
      {...props}
    />
...
function AvatarBadge({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
      ...
      className={cn(
        "absolute right-0 bottom-0 z-10 inline-flex items-center justify-center rounded-full bg-slate-600 text-white ring-2 ring-white select-none",
