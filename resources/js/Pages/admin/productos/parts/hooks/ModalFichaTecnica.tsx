...
export default function ModalFichaTecnica({ open, onClose, productoId, config }: Props) {
    ...
    function guardar() {
        setGuardando(true)
        router.post(
            route('admin.productos.hooks.config', { producto: productoId, hook: 'ficha_tecnica' }),
            { config: { titulo, descripcion, specs: specs.filter(s => s.nombre.trim()), sin_lista: sinLista } } as any,
            {
                preserveScroll: true,
